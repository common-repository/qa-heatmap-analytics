 ...
 */
namespace QAAnalyticsVendor\Google\AccessToken;

use QAAnalyticsVendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use QAAnalyticsVendor\Firebase\JWT\SignatureInvalidException;
use QAAnalyticsVendor\GuzzleHttp\Client;
use QAAnalyticsVendor\GuzzleHttp\ClientInterface;
use QAAnalyticsVendor\phpseclib3\Crypt\PublicKeyLoader;
use QAAnalyticsVendor\phpseclib3\Crypt\RSA\PublicKey;
use QAAnalyticsVendor\Psr\Cache\CacheItemPoolInterface;
use QAAnalyticsVendor\Google\Auth\Cache\MemoryCacheItemPool;
use QAAnalyticsVendor\Google\Exception as GoogleException;
use QAAnalyticsVendor\Stash\Driver\FileSystem;
use QAAnalyticsVendor\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use QAAnalyticsVendor\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('QAAnalyticsVendor\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'QAAnalyticsVendor\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('QAAnalyticsVendor\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('QAAnalyticsVendor\\phpseclib3\\Crypt\\RSA')) {
            return 'QAAnalyticsVendor\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('QAAnalyticsVendor\\phpseclib\\Crypt\\RSA')) {
            return 'QAAnalyticsVendor\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('QAAnalyticsVendor\\phpseclib3\\Math\\BigInteger')) {
            return 'QAAnalyticsVendor\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('QAAnalyticsVendor\\phpseclib\\Math\\BigInteger')) {
            return 'QAAnalyticsVendor\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('QAAnalyticsVendor\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('QAAnalyticsVendor\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('QAAnalyticsVendor\\Crypt_RSA')) {
