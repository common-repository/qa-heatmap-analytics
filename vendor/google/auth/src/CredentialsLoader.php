...
use QAAnalyticsVendor\Google\Auth\Credentials\InsecureCredentials;
...
abstract class CredentialsLoader implements FetchAuthTokenInterface, UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        ...
        if (\defined('QAAnalyticsVendor\\GuzzleHttp\\ClientInterface::VERSION')) {
    ...
    public static function makeHttpClient(FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], callable $httpHandler = null, callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \QAAnalyticsVendor\GuzzleHttp\Client($httpClientOptions);
        ...
        $stack = \QAAnalyticsVendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \QAAnalyticsVendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
